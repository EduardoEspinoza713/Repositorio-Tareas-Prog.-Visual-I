Public Class Form3
    ...
    coalesce(sc.inversion, 0.00) as inversion,
    coalesce(sv.ganancia_bruta, 0.00) as 'ganancia bruta',
    (coalesce(sv.ganancia_bruta, 0.00) - coalesce(sc.inversion, 0.00)) as 'ganancia neta'
from
    productos p
inner join
...
left join
    (
        ...
        group by
            dc.id_producto
    ) as sc on p.id_producto = sc.id_producto
left join
    (
        select
            dv.id_producto,
        ...
        group by
            dv.id_producto
    ) as sv on p.id_producto = sv.id_producto
order by
    p.id_producto;", "RGan")
            rgan.MdiParent = Me
            rgan.Show()
